...
class ReprocessarJmcPage extends StatefulWidget {
  const ReprocessarJmcPage({super.key});

  static String routeName = 'reprocessar_jmc';
  static String routePath = '/reprocessar_jmc';

  @override
  State<ReprocessarJmcPage> createState() => _ReprocessarJmcPageState();
}
...
class _ReprocessarJmcPageState extends State<ReprocessarJmcPage> {
  ...
  final List<FocusNode> _focusA = List.generate(3, (_) => FocusNode());
  final List<FocusNode> _focusB = List.generate(3, (_) => FocusNode());
  ...
  final _codA = List.generate(3, (_) => TextEditingController());
  final _codB = List.generate(3, (_) => TextEditingController());
  ...
  Future<void> _sincronizarCaixasLidasComSQLite() async {
    ...
    try {
      final resp = await ListarCaixasConferenciaCall.call(cliente: 'Jmc')
      ...
      final linhas = items
          .map<Map<String, dynamic>>((raw) {
            ...
            return <String, dynamic>{
              ...
              'Cliente': (e['Cliente'] ?? e['cliente'] ?? 'Jmc').toString(),
            };
          })
  ...
  Future<void> _carregarCaixasFromSQLite() async {
    try {
      final caixas = await SQLiteManager.instance.getDistinctCaixas('JMC');
      if (!mounted) return;
      setState(() {
        _caixas = caixas;
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // Nova Operação (API)
              FutureBuilder<List<BuscaOperacaoJMCRow>>(
                future: _novaClasseSel == null
                    ? Future.value([])
                    : SQLiteManager.instance.buscaOperacaoJMC(),
